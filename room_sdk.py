...
def get_userInRoom(chatroom_uuid, cookie, headers):
    response = requests.get(url = chatroomserviceurl + '/userinroom', params={'chatroom_uuid': chatroom_uuid, 'cookie': cookie}, headers=headers)
...
def post_message(chatroom_uuid, message, cookie, headers):
    return requests.post(url = chatroomserviceurl + '/message',  params={'chatroom_uuid': chatroom_uuid, 'cookie': cookie, 'message': message}, headers=headers)
